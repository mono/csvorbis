...
public class VorbisFile{
  ...
  public VorbisFile(String file) {
    ...
    */
    try{ inst=new SeekableInputStream(file);}
    catch(Exception e){
      throw new csorbisException("VorbisFile: "+e.toString());
    }
  ...
  public VorbisFile(InputStream inst, byte[] initial, int ibytes){
    super();
    int ret=open(insts, initial, ibytes);
    if(ret==-1){
	}
  }
...
//    return -1;
  ...
  int open(InputStream iis, byte[] initial, int ibytes){
    return open_callbacks(iis, initial, ibytes);
  }

  int open_callbacks(InputStream iis, byte[] initial, int ibytes){
    int ret;
    datasource=iis;
    ...
    if(iis is SeekableInputStream){ ret=open_seekable(); }
